...
txt_file=replacement_images2.txt
if [[ -f "$txt_file" ]]
then
	rm replacement_images2.txt
fi
...
for line in $(cat ./imagelist2.txt)
do
	echo "        <td><img src="\"images2/"$line\" alt=\"\" border=3 height=100 width=300</th>" >> replacement_images2.txt
done


# before moving forward to avoid overwriting or duplication, remove any existing file named temp_index.html
temp_html_file=temp_index2.html
if [[ -f "$temp_html_file" ]]
then
	rm temp_index2.html
fi
...
num_images=$(find ~/projects/MIDS1D/website/html/images2 -maxdepth 1 -type f | wc -l)
...
for line in $(cat temp_index2.html)
do
declare firstline=$(head -n 1 replacement_images2.txt)
	if [[ $line == "replace" ]]
	then
		echo $line >> index.html; sed -i "s,replace,$firstline," index.html; sed -i '1d' replacement_images2.txt
	else
